...
public class ProjectJpaRepository /* extends JpaRepository<ProjectEntity, UUID> */ {
  ...
  // investmentId
  // @Query("SELECT p FROM ProjectEntity p LEFT JOIN FETCH p.investments i WHERE
  // i.id = :investmentId")
  ...
  // Optional<ProjectEntity> findByInvestmentId(@Param("investmentId") UUID
  // investmentId);
  ...
  // @Query("SELECT p FROM ProjectEntity p WHERE p.status = :projectStatus AND
  // p.id NOT IN (SELECT DISTINCT i.project.id FROM InvestmentEntity i WHERE
  // i.status = :investmentStatus)")
  // Optional<List<ProjectEntity>> findByStatusAndNotHavingInvestmentStatus(
  // @Param("projectStatus") String projectStatus,
  // @Param("investmentStatus") String investmentStatus);
